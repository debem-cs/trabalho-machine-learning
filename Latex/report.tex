...
\begin{figure}[H]
\centering
\begin{lstlisting}[language=Python, caption=Python implementation of temporal difference features]
# Temporal Differences Calculation
for i in range(4):
    date_curr = f'date{i}'
    date_next = f'date{i+1}'
    
    for band in ['red', 'green', 'blue']:
        curr_col = f'img_{band}_mean_{date_curr}'
        next_col = f'img_{band}_mean_{date_next}'
        
        # Difference (Magnitude of change)
        diff_col = f'diff_{band}_mean_{i}_{i+1}'
        df[diff_col] = df[next_col] - df[curr_col]
\end{lstlisting}
\end{figure}
